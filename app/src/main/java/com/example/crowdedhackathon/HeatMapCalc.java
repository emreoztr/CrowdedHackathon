...
public class HeatMapCalc{
    ...
    HeatMapCalc(GoogleMap map, ArrayList<LatLng> markers, double rad, double collVal) throws Exception {
        ...
        correctLongitude = 111320*Math.cos(marker_1.latitude);
        _rad = rad;
        ...
        for (int i = 0; lenbtCircles * i < crossLen + rad; ++i) { //
            ...
            for (int j = 0; (lenbtCircles) * j < relEdgeLen + rad; ++j) {
                ...
                circleLoc.add(new LatLng(
                        marker_1.latitude + (lenbtCircles * j) * Math.cos(Math.toRadians(45.0) + alpha)/correctLatitude,
                        marker_1.longitude + (lenbtCircles * j) * Math.sin(Math.toRadians(45.0) + alpha)/correctLongitude));

                //adds to other side
                circleLoc.add(new LatLng(
                        marker_1.latitude + (lenbtCircles * j) * Math.cos(Math.toRadians(135.0) + alpha)/correctLatitude,
                        marker_1.longitude + (lenbtCircles * j) * Math.sin(Math.toRadians(135.0) + alpha)/correctLongitude));
            }
        }
    ...
    public double calcSlope() {
        return (((marker_2.longitude - marker_1.longitude)*correctLongitude) / ((marker_2.latitude - marker_1.latitude)*correctLatitude));
    }
